...
This Ansible Role installs and configures the duo auth proxy for LDAP and RADIUS use, as well as user-sync.

Installation with One-Line-setup:

``` bash
curl https://raw.githubusercontent.com/uniQconsulting-ag/ansible.duo/master/setup.sh | sh
```

Installation with ansible-galaxy:

``` bash
ansible-galaxy install uniqconsulting.duo
```
